...
export function useGameStateMigration() {
  ...
  const migrateToMultiplayer = useCallback(
    async (roomId: string, migrationData: MigrationData) => {
      try {
        ...
        await migrateGameState(roomId, {
          ...
          clues: {
            ...clueStatesForFirebase.rowClues.reduce(
              (acc, clueId) => ({ ...acc, [clueId]: true }),
              {}
            ),
            ...clueStatesForFirebase.colClues.reduce(
              (acc, clueId) => ({ ...acc, [clueId]: true }),
              {}
            ),
          },

...
export function useGameState(
  ...
  const handleCellMouseDown = useCallback(
    (position: CellPosition, button: number) => {
    ...
    (position: CellPosition) => {
      ...
      if (
        ...
        if (currentRow === startRow) {
          ...
          dragState.current.direction =
            rowDiff > colDiff ? "vertical" : "horizontal";
        }
      }
      ...
      cellsToModify.forEach((cell) => {
        if (
          cell.row >= 0 &&
          cell.row < puzzle!.size.height &&
          cell.col >= 0 &&
          cell.col < puzzle!.size.width
        ) {
  ...
  const applyCellChange = useCallback(
    async (position: CellPosition) => {
      if (
        !puzzle ||
        !dragState.current.startState ||
        !dragState.current.startCell
      )
        return;
      ...
      setGameState((prev) => {
        ...
        } else if (dragState.current.button === 1) {
          // Middle button
          newState = getNextState(
            "o",
            dragState.current.startState!,
        ...
        } else if (dragState.current.button === 2) {
          // Right button
          newState = getNextState(
            "x",
            dragState.current.startState!,
  ...
  const updateCellExternally = useCallback(
    (cellId: string, newState: CellState) => {
      ...
      setGameState((prev) => {
        ...
        const isComplete = puzzle
          ? checkSolution(newGrid, puzzle.solution)
          : false;
